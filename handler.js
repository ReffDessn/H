                    ...
                    for (let user of participants) {
                       let pp = './src/welcome.jpg'
                        try {
                            ...
                            text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'ʜᴀʏ ᴅᴇᴄᴋ 👋').replace('@subject', groupMetadata.subject).replace('@desc', groupMetadata.desc.toString()) :
                                (chat.sBye || this.bye || conn.bye || 'Anggota Group Berkurang -1'))
                                this.sendButtonImg(id, pp, text, "© ʏᴀɴɴ-ʙᴏᴛᴢ", "ʏ", "wkwk", null)
                                }
                    }
                }
                break
            case 'promote':
                text = (chat.sPromote || this.spromote || conn.spromote || 'Promote Detect, @user ```is now Admin```')
            case 'demote':
                if (!text) text = (chat.sDemote || this.sdemote || conn.sdemote || 'Demote Detect, @user ```is no longer Admin```')
...
global.dfail = (type, m, conn) => {
    let msg = {
        rowner: '*ғɪᴛᴜʀ ɪɴɪ ᴋʜᴜsᴜs ᴏᴡɴᴇʀ ! *',
        owner: '*ғɪᴛᴜʀ ɪɴɪ ᴋʜᴜsᴜs ᴏᴡɴᴇʀ*',
        mods: '*ʜᴀɴʏᴀ ᴅᴀᴘᴀᴛ ᴅɪ ɢᴜɴᴀᴋᴀɴ ᴏʟᴇʜ ᴍᴏᴅᴇʀᴀᴛᴏʀ ʙᴏᴛ*_ !',
